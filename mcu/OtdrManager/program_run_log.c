...
#ifdef ARM_BOARD
const char self_path[] = "./targetExe-r";
#else
const char self_path[] = "./OtdrManager";
#endif
/*
         **************************************************************************************
         *  函数名称：按照当前格式获取系统时间
...
int get_self_modify_time(char time_buf[])
{
    ...
    struct stat file_stat;
    time_t modify_time;
    struct tm tm_modify, *ptm;
    const char* pFormatDate = "V%Y.%m.%d";
    ret = -1;
    //打开文件目录
    ret = stat(self_path,&file_stat);
    if(ret != 0){
        printf("%s(): Line : %d get file %s stat error %d \n",  \
               __FUNCTION__, __LINE__,self_path,errno);
    }
    else{
        //posix2008版本以后，已经没有st_mtime,修改为新类型timespec
        //如果日志不能删除，检查这里
        modify_time = file_stat.st_mtime;
        ptm = gmtime_r((const time_t *)(&modify_time), &tm_modify);
        if(ptm == NULL){
            printf("%s(): Line : %d get file %s stat error %d \n",  \
                   __FUNCTION__, __LINE__,self_path,errno);
            ret = -1;
        ...
        strftime(time_buf, 20, pFormatDate, &tm_modify);
        ret = RET_SUCCESS;
    }
