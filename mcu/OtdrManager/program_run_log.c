...
int check_log_head( _tagLogHead log_head)
{
    ...
    if((log_head.cur_index < 0)
            || (log_head.cur_index >= MSG_BLOCK_SIZE)) {
        ret = -1;
        goto usr_exit;
    }
    //日志块字节数 信息部分+时间字符串
...
{    
    ...
    if((bytes == 0) ||
        ...
        //指向下一个存储位置，
        log_head.cur_index ++;
        if(  log_head.cur_index >=  log_head.block_size)
            log_head.cur_index = 0;
        if(log_head.cur_num < log_head.block_size)
            log_head.cur_num ++;

    }
    //保存头信息
    fseek(fp, 0, SEEK_SET);
    fwrite(&log_head, sizeof(log_head),1,fp);
...
int convert_log2txt(char file_path[])
{
    ...
    for(i = 0; i < log_head.cur_num;i++)
    {
        ...
        //保存成文本
        fprintf(fp_txt, "%s ------ ", time_buf);
        fprintf(fp_txt, "Fuction %s ------ ", msg.function);
        fprintf(fp_txt, "Line %d ------ ", msg.line);
        fprintf(fp_txt, " %s ------ ", msg.log_msg);
    }
